...
class PremierLeagueService
{
    ...
    {
        $seasonId = config('services.schibsted_sports.premier_league_season_id');
    ...
    {
        $tournamentId = config('services.schibsted_sports.premier_league_tournament_id');
    ...
    {
        $cacheKey = sprintf('schibsted_sports.premier_league.%d.%s', $seasonId, $endpoint);
        $ttl = (int) config('services.schibsted_sports.cache_ttl', 900);

        return Cache::remember($cacheKey, now()->addSeconds($ttl), function () use ($seasonId, $endpoint) {
            try {
    ...
    {
        return 'schibsted_sports.premier_league.'.$seasonId.'.last_good';
    }

    private function logApiFailure(Throwable $exception): void
    {
        Log::warning('Could not fetch Premier League data from Schibsted Sports API.', [
