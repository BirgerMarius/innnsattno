...
<div class="cl-page"><div class="container py-4 py-md-5">
    ...
    <section class="cl-section p-3 p-md-4 mb-3">
        ...
        @foreach($standings as $team)<tr><td>{{ $team['rank'] ?? '-' }}</td><td><span class="cl-team">@if($team['emblemUrl'])<img class="cl-emblem" src="{{ $team['emblemUrl'] }}" alt="" onerror="this.remove()">@endif{{ $team['teamName'] }}</span></td><td class="text-center">{{ $team['played'] ?? '-' }}</td><td class="text-center">{{ $team['wins'] ?? '-' }}</td><td class="text-center">{{ $team['draws'] ?? '-' }}</td><td class="text-center">{{ $team['losses'] ?? '-' }}</td><td class="text-center">{{ $team['goalsFor'] ?? '-' }}-{{ $team['goalsAgainst'] ?? '-' }}</td><td class="text-center">{{ $team['goalDifference'] ?? '-' }}</td><td class="text-center font-weight-bold">{{ $team['points'] ?? '-' }}</td></tr>@endforeach
        </tbody></table></div>
