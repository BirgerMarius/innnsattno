...
.page-break { break-before:page; page-break-before:always; } .fixtures { display:grid; gap:5mm; grid-template-columns:repeat(2,minmax(0,1fr)); } .match { border-bottom:.5px solid #aaa; break-inside:avoid; page-break-inside:avoid; padding:1.4mm 0; } .match-teams { overflow-wrap:anywhere; } .phase { color:#444; font-size:8pt; } .knockout-round { break-inside:avoid; page-break-inside:avoid; margin:0 0 4mm; } .tie { border:1px solid #999; margin:0 0 2mm; padding:1.5mm; } .leg { border-top:.5px solid #ddd; padding:1mm 0; } .leg:first-child { border-top:0; padding-top:0; } .aggregate { font-weight:bold; margin:1mm 0 0; } .warning,.empty { border:1px solid #999; padding:2.5mm; }
.football-tv-box { border:1px solid #777; break-inside:avoid; margin:0 0 3mm; padding:2mm 2.5mm; page-break-inside:avoid; } .football-tv-box h2 { font-size:10pt; margin:0; } .football-tv-box__selection { font-size:8pt; margin:.5mm 0 1mm; } .football-tv-box__match { border-top:.5px solid #aaa; padding:.5mm 0; } .football-tv-box__match span { display:block; font-size:8pt; } .football-tv-box__status,.football-tv-box__note { font-size:8pt; margin:1mm 0 0; }
...
@include('football.partials.tv-matches-box', ['tvBoxContext' => 'uefa-competition-print'])
...
<section class="page-break"><h2>Kamper</h2><div class="fixtures"><div><h3>Siste resultater</h3>@forelse($printResults as $match)<div class="match"><div class="phase">{{ $match['round'] ?: 'Ferdigspilt' }} · {{ $match['dateLabel'] }}</div><div class="match-teams">@if($match['homeEmblemUrl'])<img class="emblem" src="{{ $match['homeEmblemUrl'] }}" alt="" onerror="this.remove()">@endif{{ $match['homeTeam'] }} <strong>{{ $match['homeScore'] ?? '–' }}–{{ $match['awayScore'] ?? '–' }}</strong> @if($match['awayEmblemUrl'])<img class="emblem" src="{{ $match['awayEmblemUrl'] }}" alt="" onerror="this.remove()">@endif{{ $match['awayTeam'] }}</div></div>@empty<p class="empty">Ingen ferdigspilte kamper de siste 7 døgnene.</p>@endforelse</div><div><h3>Kommende kamper</h3>@forelse($printFixtures as $match)<div class="match"><div class="phase">{{ $match['startsAt']->locale('nb')->translatedFormat('D j. M \k\l. H.i') }}@if($match['round']) · {{ $match['round'] }}@endif</div><div class="match-teams">@if($match['homeEmblemUrl'])<img class="emblem" src="{{ $match['homeEmblemUrl'] }}" alt="" onerror="this.remove()">@endif{{ $match['homeTeam'] }} – @if($match['awayEmblemUrl'])<img class="emblem" src="{{ $match['awayEmblemUrl'] }}" alt="" onerror="this.remove()">@endif{{ $match['awayTeam'] }}</div></div>@empty<p class="empty">Ingen kamper er satt opp de neste 7 døgnene.</p>@endforelse</div></div></section>
